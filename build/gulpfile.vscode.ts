		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(path.join(cwd, dep), {
				...
				'version-string': {
					...
					'LegalCopyright': 'Copyright (C) 2026 Trixty. All rights reserved',
...
function copyTrixtyAINativeDepsTask(platform: string, arch: string, destinationFolderName: string) {
	return async () => {};
}
