...
AppPublisher=Trixty
AppPublisherURL=https://trixty.vercel.app/ide
AppSupportURL=https://trixty.vercel.app/ide
AppUpdatesURL=https://trixty.vercel.app/ide
DefaultGroupName={#NameLong}
AllowNoIcons=yes
OutputDir={#OutputDir}
OutputBaseFilename=TrixtySetup
Compression=lzma
...
// to shutdown Trixty. We rely on the fact that the update signals
// that Trixty is ready to be shutdown, so we're good to use `force` here.
...
begin
  Result := True;
    ...
    if not WizardSilent() and IsAdmin() then begin
      if MsgBox('This User Installer is not meant to be run as an Administrator. If you would like to install Trixty for all users in this system, download the System Installer instead from https://code.visualstudio.com. Are you sure you want to continue?', mbError, MB_OKCANCEL) = IDCANCEL then begin
        Result := False;
      end;
    end;
  #endif
...
// Trixty will create a flag file before the update starts (/update=C:\foo\bar)
...
// Check if Trixty created a session-end flag file to indicate OS is shutting down
...
// Check if Trixty created a cancel file to signal that the update should be aborted
...
begin
  ...
  // During background updates, also create a -updating mutex that Trixty checks
