		...
		const updateRegistration = () => {
			...
			if (!disabled) {
				...
				if (!context.state.installed && !otherAgentsAndToolsDisposable.value) {
					...
					// VSCode Agent
					disposables.add(SetupChatAgent.registerOtherAgents(this.instantiationService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about Trixty IDE").value, ChatAgentLocation.Panel, undefined, context, controller).disposable);
