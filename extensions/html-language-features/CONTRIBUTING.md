...
- Open `/extensions/html-language-features/` as the workspace in Trixty IDE
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/html-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Trixty IDE instance with the `html-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Trixty IDE window opened on `html-language-features`.
...
This extension wraps the html language service into a Language Server for Trixty IDE.
