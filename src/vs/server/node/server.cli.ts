...
/*
 * Implements a standalone CLI app that opens Trixty IDE from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Trixty IDE on the Windows side.
 *    The Trixty IDE desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to Trixty IDE server: ' + message);
	console.error(err);
	process.exit(1);
}
