...
export class DarwinUpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected buildUpdateFeedUrl(quality: string, commit: string, options?: IUpdateURLOptions): string | undefined {
		...
		const headers: Record<string, string> = getUpdateRequestHeaders(this.productService.version) || {};
		try {
	...
	private async checkForUpdateNoDownload(url: string, canInstall?: boolean): Promise<void> {
		...
		try {
			...
			const githubRelease = await asJson<any>(context);
			if (!githubRelease || !githubRelease.tag_name) {
				...
				let macAsset = assets.find((a: any) => a.name.endsWith('.dmg') || a.name.endsWith('.zip'));
				
				const update: IUpdate = {
					version: version,
					productVersion: version,
					url: macAsset ? macAsset.url : (githubRelease.html_url || githubRelease.url)
				};
