...
export class Win32UpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected buildUpdateFeedUrl(quality: string, commit: string, options?: IUpdateURLOptions): string | undefined {
		// Used to fallback if needed, but we will override doCheckForUpdates directly
		if (this.productService.githubUpdateRepo) {
			return `https://api.github.com/repos/${this.productService.githubUpdateRepo}/releases/latest`;
		}
	...
	protected doCheckForUpdates(explicit: boolean, pendingCommit?: string): void {
		...
		let headers: Record<string, string> = getUpdateRequestHeaders(this.productService.version) || {};
		...
		this.requestService.request({ url, headers, callSite: 'updateService.win32.checkForUpdates' }, CancellationToken.None)
			...
			.then(githubRelease => {
				...
				if (!githubRelease || !githubRelease.tag_name) {
					// Fallback for not found or empty
					if (this.state.type === StateType.Overwriting) {
				...
				// Compare version string simply. E.g '1.5.0' vs '1.6.0'
				const currentVersion = this.productService.version;
				const releaseVersion = githubRelease.tag_name.replace(/^v/, '');
				...
				let winAsset = assets.find((a: any) => a.name.endsWith('.exe') && (this.productService.target === 'user' ? a.name.indexOf('User') > -1 : a.name.indexOf('System') > -1));
				
				if (!winAsset && assets.length > 0) {
					// Fallback to any exe if exact match not found
					winAsset = assets.find((a: any) => a.name.endsWith('.exe'));
				}
				...
				const update: IUpdate = {
					version: releaseVersion, // commit id in github releases
				...
				return this.cleanup(update.version).then(() => {
					return this.getUpdatePackagePath(update.version).then(updatePackagePath => {
						return pfs.Promises.exists(updatePackagePath).then(exists => {
							...
							if (this.productService.githubUpdateToken) {
								dlHeaders['Authorization'] = `Bearer ${this.productService.githubUpdateToken}`;
							}

							return this.requestService.request({ url: update.url!, headers: dlHeaders, callSite: 'updateService.win32.downloadUpdate' }, CancellationToken.None)
								.then(context => {
			...
			.then(undefined, err => {
				// Handle specific GitHub errors for Token Expired notification
