...
export class EditTrackingFeature extends Disposable {
	...
	constructor(
		...
		const copilotInstalled = getExtensionInfoObs('Trixty.trixty-ai', this._extensionService);
		const copilotChatInstalled = getExtensionInfoObs('Trixty.trixty-ai-chat', this._extensionService);
