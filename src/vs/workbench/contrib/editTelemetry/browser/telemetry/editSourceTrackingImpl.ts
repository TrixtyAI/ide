...
class TrackedDocumentInfo extends Disposable {
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		const getEditCategory = (source: EditSource) => {
			...
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'trixty.ai') { return 'inlineCompletionsCopilot'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'trixty.ai-chat' && source.providerId === 'completions') { return 'inlineCompletionsCopilot'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'trixty.ai-chat' && source.providerId === 'nes') { return 'inlineCompletionsNES'; }
