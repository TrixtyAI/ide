...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, `${defaultChat.provider.default.name} AI` /* Do NOT change, this hides the username altogether in Chat */, true, description, location, mode, context, controller);
		});
	}
	...
	static registerBuiltInAgents(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): IDisposable {
		return instantiationService.invokeFunction(accessor => {
			...
			const { disposable: vscodeDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about Trixty IDE").value, ChatAgentLocation.Chat, ChatModeKind.Agent, context, controller);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				modelDescription: 'Scaffold a new workspace in Trixty IDE',
				userDescription: localize('setupToolsDescription', "Scaffold a new workspace in Trixty IDE"),
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpCopilotNeeded', "You need to set up Trixty AI and be signed in to use Chat."));
	...
	private replaceToolInRequestModel(requestModel: IChatRequestModel): IChatRequestModel {
		...
		const toolId = toolPart.toolId.replace(/setup.tools\./, `trixty_`.toLowerCase());
...
export class ChatCodeActionsProvider {
	...
	async provideCodeActions(model: ITextModel, range: Range | Selection): Promise<CodeActionList | undefined> {
		...
		if (generateOrModifyTitle && generateOrModifyCommand) {
			actions.push({
				kind: CodeActionKind.RefactorRewrite.append('trixty').value,
				isAI: true,
		...
		if (markers.length > 0) {
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('trixty').value,
				isAI: true,
				diagnostics: markers,
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('explain').append('trixty').value,
				isAI: true,
				diagnostics: markers,
