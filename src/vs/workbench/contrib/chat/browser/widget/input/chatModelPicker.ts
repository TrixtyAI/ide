...
function getUpdateHoverContent(updateState: StateType): MarkdownString {
	...
	switch (updateState) {
		case StateType.AvailableForDownload:
			hoverContent.appendMarkdown(localize('chat.modelPicker.downloadUpdateHover', "This model requires a newer version of Trixty IDE. [Download Update](command:update.downloadUpdate) to access it."));
			break;
		case StateType.Downloaded:
		case StateType.Ready:
			hoverContent.appendMarkdown(localize('chat.modelPicker.restartUpdateHover', "This model requires a newer version of Trixty IDE. [Restart to Update](command:update.restartToUpdate) to access it."));
			break;
		default:
			hoverContent.appendMarkdown(localize('chat.modelPicker.checkUpdateHover', "This model requires a newer version of Trixty IDE. [Update Trixty IDE](command:update.checkForUpdate) to access it."));
			break;
	}
	return hoverContent;
}
...
export function buildModelPickerItems(
	...
	if (useGroupedModelPicker) {
		...
		if (models.length) {
			...
			// --- 1. Auto ---
			const autoModel = models.find(m => m.metadata.id === 'auto' && m.metadata.vendor === 'trixty');
			if (autoModel) {
			...
			otherModels = models
				...
				.sort((a, b) => {
					...
					const aCopilot = a.metadata.vendor === 'trixty' ? 0 : 1;
					const bCopilot = b.metadata.vendor === 'trixty' ? 0 : 1;
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		description = localize('chat.modelPicker.updateDescription', "Update Trixty IDE");
	} else {
	...
	if (reason === 'upgrade') {
		...
		hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHover', "[Upgrade Trixty AI Plan](command:workbench.action.chat.upgradePlan \" \") to use the best models."));
	} else if (reason === 'update') {
	...
	show(anchor?: HTMLElement): void {
		...
		const onSelect = (model: ILanguageModelChatMetadataAndIdentifier) => {
			this._telemetryService.publicLog2<ChatModelChangeEvent, ChatModelChangeClassification>('chat.modelChange', {
				fromModel: previousModel?.metadata.vendor === 'trixty' ? new TelemetryTrustedValue(previousModel.identifier) : 'unknown',
				toModel: model.metadata.vendor === 'trixty' ? new TelemetryTrustedValue(model.identifier) : 'unknown'
			});
			this._selectedModel = model;
			this._renderLabel();
...
function getModelHoverContent(model: ILanguageModelChatMetadataAndIdentifier, languageModelsService?: ILanguageModelsService): MarkdownString {
	const isAuto = model.metadata.id === 'auto' && model.metadata.vendor === 'trixty';
