...
const COPILOT_EXTENSION_IDS = ['trixty.ai', 'trixty.ai-chat'];
...
class McpServerItemRenderer implements IListRenderer<IMcpServerItemEntry | IMcpBuiltinItemEntry, IMcpServerItemTemplateData> {
	...
	renderElement(element: IMcpServerItemEntry | IMcpBuiltinItemEntry, index: number, templateData: IMcpServerItemTemplateData): void {
		...
		templateData.disposables.add(this.hoverService.setupManagedHover(
			...
			localize('bridgedHover', "This server is managed by Trixty IDE and forwarded to all compatible agent sessions."),
		));
...
export class McpListWidget extends Disposable {
	...
	private filterServers(): void {
		...
		// Servers from the Copilot extension (trixty.ai / trixty.ai-chat)
		...
		if (extensionServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('extensionGroupDescription', "MCP servers contributed by installed Trixty IDE extensions."),
				collapsed,
			});
			if (!collapsed) {
		...
		if (otherBuiltinServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('builtInGroupDescription', "MCP servers built into Trixty IDE. These are available automatically."),
				collapsed,
			});
			if (!collapsed) {
