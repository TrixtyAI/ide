...
const OPEN_REPO_COMMAND = 'trixty.ai.chat.cloudSessions.openRepository';
...
export const COPILOT_MULTI_CHAT_SETTING = 'sessions.trixty.ai.multiChatSessions';
...
export class CopilotChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	async deleteSession(sessionId: string): Promise<void> {
		...
		if (agentSession) {
			if (agentSession.providerType === CopilotCLISessionType.id) {
				this.commandService.executeCommand('trixty.ai.cli.sessions.delete', { resource: agentSession.resource });
			} else {
			...
			for (const chatId of chatIds) {
				...
				if (chatSession) {
					if (chatSession.providerType === CopilotCLISessionType.id) {
						this.commandService.executeCommand('trixty.ai.cli.sessions.delete', { resource: chatSession.resource });
					} else {
	...
	async renameChat(sessionId: string, _chatUri: URI, title: string): Promise<void> {
		...
		if (agentSession) {
			if (agentSession.providerType === CopilotCLISessionType.id) {
				this.commandService.executeCommand('trixty.ai.cli.sessions.setTitle', { resource: agentSession.resource }, title);
			} else {
