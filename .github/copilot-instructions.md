# Trixty IDE Copilot Instructions
...
- `extensions/`: Built-in extensions that ship with Trixty IDE
...
- `src/vs/workbench/` - Main application workbench for web and desktop
  ...
  - `workbench/api/` - Extension host and Trixty IDE API implementation
...
The `extensions/` directory contains first-party extensions that ship with Trixty IDE:
...
Each extension follows the standard Trixty IDE extension structure with `package.json`, TypeScript sources, and contribution points to extend the workbench through the Extension API.
...
- If the `#runTasks/getTaskOutput` tool is available, check the `Trixty IDE - Build` watch task output for compilation errors. This task runs `Core - Build` and `Ext - Build` to incrementally compile Trixty IDE TypeScript sources and built-in extensions. Start the task if it's not already running in the background.
- If the tool is not available (e.g. in CLI environments) and you only changed code under `src/`, run `npm run compile-check-ts-native` after making changes to type-check the main Trixty IDE sources (it validates `./src/tsconfig.json`).
