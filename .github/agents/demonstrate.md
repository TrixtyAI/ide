---
name: Demonstrate
description: Agent for demonstrating Trixty IDE features
...
5. Start Trixty IDE automation using `vscode_automation_start`
...
## Pointers for Controlling Trixty IDE

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for Trixty IDE interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout Trixty IDE) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
