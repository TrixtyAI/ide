...
export const ExtensionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshCatalog = async () => {
    ...
    try {
      ...
      const catalogData = await invoke<any>("get_registry_catalog", { url: targetUrl });
      let entries: MarketplaceEntry[] = catalogData.marketplace || [];
      ...
      const enrichedEntries = await Promise.all(
        entries.map(async (entry) => {
          try {
            const manifest = await invoke<ExtensionManifest>("fetch_extension_manifest", { 
      ...
      for (const id of installed) {
        const isAct = await invoke<boolean>("is_extension_active", { id });
  ...
  const fetchFile = async (entry: MarketplaceEntry, fileName: string) => {
      try {
        const text = await invoke<string>("fetch_extension_file", { 
